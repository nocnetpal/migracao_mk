...
- `09-gerencia-remota.rsc`: winbox/ssh passam a aceitar bancada (`192.168.88.0/24`) + `/27`
  ...
  -> ✅ corrigido: analise anterior errada — o default do ovpn-server e `disabled=yes` e a entrada
  `ovpn-server1` no export e artefato automatico da migracao 7.17+ (doc oficial); ja vem
  desabilitado, nada a fazer.
